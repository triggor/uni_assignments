 ...
 * Igor Tryhub
 * ./randwalk -n 7 -s 15 -t  14 -v 0
 * Time elapsed: 12.000934 seconds.
 * ./randwalk -n 7 -s 15 -t  14 -v 1
 * Time elapsed: 6.179272 seconds.
 */
int randwalk2(uint8_t *arr, int n, int len) {
  ...
  int i_dependent[4] = {-1, 1, 0, 0};
  int j_dependent[4] = {0, 0, -1, 1};
  ...
  do {
    ...
    sum += arr[i * n + j];
    int remainder = (dir >> k) & 3;
    
    i_dependent[0] = (i > 0 ? -1 : 0);
    i_dependent[1] = (i < n-1 ? 1 : 0);
    i += i_dependent[remainder];
    
    j_dependent[2] = (j > 0 ? -1 : 0);
    j_dependent[3] = (j < n-1 ? 1 : 0);
    j += j_dependent[remainder];
...
int main(int argc, char **argv) {
  ...
  if (err || size < 0 || steps < 0 || times < 0 || var < 0 || var >= 2) {
    fprintf(stderr, "Usage: %s -n log2(size) -s log2(length) -t log2(times) "
                    "-v variant\n", argv[0]);
    exit(EXIT_FAILURE);
  }
